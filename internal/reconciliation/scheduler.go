...
// Scheduler periodically checks SSO-bound admin identities against the IdP
// and disables any admins whose subjects are no longer active.
...
func (s *Scheduler) reconcile(ctx context.Context) {
	...
	for _, admin := range admins {
		...
		if status.Error != nil {
			// Fail-open: IdP unreachable → don't disable anyone
		...
		if !status.Active {
			...
			s.auditLog(ctx, admin.OrgID, admin.ID, admin.SSOSubject)
		}
	}
}

func (s *Scheduler) auditLog(ctx context.Context, orgID, adminID, ssoSubject string) {
	if s.auditor == nil {
		return
	}
	_ = s.auditor.Log(ctx, &models.AuditEvent{
		...
		Timestamp: time.Now(),
		OrgID:     orgID,
		EventType: models.AuditEventType("admin.reconciliation.disabled"),
